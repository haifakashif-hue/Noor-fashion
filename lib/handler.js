...
const { routes, HttpError, RawResponse } = require("./api");

const PUBLIC_DIR = path.join(__dirname, "..", "public");
const MAX_BODY = 100 * 1024; // 100 KB is plenty for a cart or a form
const HARD_BODY_CEILING = 25 * 1024 * 1024; // beyond this, stop reading and drop the connection
...
function readBody(req, limit = MAX_BODY) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      size += chunk.length;
      if (size > limit) {
        // Throw the bytes away but let the upload finish, so the client gets a
        // readable 413 instead of a reset connection. Past the hard ceiling,
        // stop listening altogether.
        tooLarge = true;
        chunks.length = 0;
        if (size > HARD_BODY_CEILING) req.destroy();
        return;
      }
      chunks.push(chunk);
    });
    req.on("end", () => {
      if (tooLarge) return reject(new HttpError(413, "That upload is too large"));
...
function matchRoute(method, pathname) {
  ...
  for (const [routeMethod, pattern, handler, options = {}] of routes) {
    ...
    if (!ok) continue;
    if (!hasParam) return { handler, params, options };
    if (!fallback) fallback = { handler, params, options };
  }
  return fallback;
}
...
async function handler(req, res) {
  ...
  try {
    ...
    const ctx = {
      ...
      body: ["POST", "PUT", "PATCH"].includes(req.method) ? await readBody(req, route.options.maxBody) : {},
